...
%% create mot files to apply torque
...
% for jj = 1
    ...
    Tx = F_mag*sind(-1*theta);
    Ty = F_mag*cosd(-1*theta);
    ...
    for i = 1:size(time,1)
        ...
        elseif (time(i)>2) & (time(i)<=14.5)
            femur_r_force_tx(i,1) = 0.5*Tx*cos((1/6.3)*pi*(time(i)-2))-0.5*Tx;
            femur_r_force_ty(i,1) = 0.5*Ty*cos((1/6.3)*pi*(time(i)-2))-0.5*Ty;
        elseif (time(i)>14.5) & (time(i)<=16.5)
            femur_r_force_tx(i,1) = 0;
            femur_r_force_tx(i,1) = 0;
        else
            femur_r_force_tx(i,1) = -1*(0.5*Tx*cos((1/5.8)*pi*(time(i)-16.5))-0.5*Tx);
            femur_r_force_ty(i,1) = -1*(0.5*Ty*cos((1/5.8)*pi*(time(i)-16.5))-0.5*Ty);
        end
    end
    ...
    file = ['../IE_Laxity/ie_' num2str(-1*ap_angs(jj,1)) '.mot'];
    ...
    saveas(gcf,['../IE_Laxity/TorqueApplied_ie' num2str(-1*ap_angs(jj,1)) '.bmp']);
    close(gcf);
