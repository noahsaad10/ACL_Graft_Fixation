function [fout, vout, cout] = load_stlFile(filename)
% [fout, vout, cout] = load_stlFile(filename)
%
% Loads an ASCII STL File *stl-file to an *.asc file.
% Paramter is the filename (without extension)
%
% USAGE:
%

% open stl file
[fout,vout,cout] = rndread([filename,'.stl']);
...
% ************************************************
% First cound the number of lines
...
% ************************************************
% Now dimension the c and v matrices and start over
v=zeros(3,npts);
c=zeros(3,npts);
fid=fopen(filename, 'r'); %Open the file, assumes STL ASCII format.
if fid == -1 
